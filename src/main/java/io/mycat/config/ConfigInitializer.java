...
public class ConfigInitializer {
	...
	public ConfigInitializer(boolean loadDataHost) {
		...
		ConfigLoader configLoader2 = new RestConfigLoader();
		schemaLoader = null;
		
		//加载配置
		this.system = configLoader.getSystemConfig();
		this.schemas = configLoader2.getSchemaConfigs();
		this.users = configLoader2.getUserConfigs();

		//是否重新加载DataHost和对应的DataNode
		if (loadDataHost) {
			this.dataHosts = initDataHosts(configLoader2);
			this.dataNodes = initDataNodes(configLoader2);
		}
